...
const OptionChip: FC<OptionChipProps> = ({ name, value, type, label }) => {
  ...
  const indicatorBase = cn(
    "flex size-5 shrink-0 items-center justify-center border transition-all duration-200 peer-focus-visible:ring-2 peer-focus-visible:ring-white/60",
  ...
  return (
    <label
      className={cn(
        "flex cursor-pointer items-center gap-3 rounded-xl border px-3.5 py-3 transition-all duration-200 active:scale-[0.98]",
        checked
    ...
    >
      ...
      {type === "checkbox" ? (
        <span aria-hidden="true" className={cn(indicatorBase, "rounded-md")}>
          <FaCheck
            className={cn(
        ...
        <span aria-hidden="true" className={cn(indicatorBase, "rounded-full")}>
          <span
            className={cn(
              "size-2 rounded-full bg-white transition-all duration-200",
